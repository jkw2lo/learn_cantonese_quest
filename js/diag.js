...
/* Named on window as well as in scope, and not as a nicety: a top-level
   ...
   a property of window, so every `window.CQDIAG &&` guard in app.js — written
   exactly so the app survives this file going missing — would read as
   "missing" and quietly record nothing. The guards are right; a plain export
   would be what was wrong. */
const CQDIAG = window.CQDIAG = (() => {
  const KEY = "cantonese-quest-log";
