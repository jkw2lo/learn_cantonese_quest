/* ============================================================
   速練 Sprint — minute math, for characters
   ...
   that say nothing about whether you know a character. They go in the 錯字本
   instead, where they can be worked on at a sensible speed.
   ============================================================ */

const SPRINT = {
  l: { zh: "聽力", name: "Listening", skill: "p", verb: "hear",
       blurb: "Hear it, pick the character.",
       long: "The headline mode, and the one this app is built around: Cantonese is a language you meet spoken, and the tones only become real when you have to tell them apart at speed.",
       par: 3.2 },
  r: { zh: "認讀", name: "Reading", skill: "r", verb: "read",
...
/* − value + , where each button is a plain data-sp-set like every chip. The
   value is clamped to the range the chips cover, and a button that would not
   move is disabled rather than hidden, so the control never changes width
   under your thumb. */
...
function sprintDistractors(ch, known, n, kind) {
  ...
  if (kind === "spot") {
    ...
    /* then the sound-alikes, which in Cantonese means same syllable, any tone
       — 買 maai5 and 賣 maai6 are one tone apart and mean opposite things */
    if (near.length < n) near = near.concat(others.filter(x => untoned(x.p) === untoned(ch.p)));
  } else if (kind === "listen") {
    /* the point is hearing it, not splitting hairs between two third tones */
    near = others.filter(x => toneOf(x.p) !== toneOf(ch.p) || untoned(x.p)[0] !== untoned(ch.p)[0]);
  }
...
/* 預備 — three seconds to put your hand on the keys. The clock starts after
   ...
   backgrounded, browsers throttle them hard, and the sheet would sit on 預備
...
function sprintReady() {
  ...
  $("#spInner").innerHTML = `<div class="sp-ready">
    <span class="sp-ready-k han">預備</span>
...
/* ---------- typing: a stripped-down IME ----------
   ...
   six 時 是 事 市 試 士 shapes on screen is the one that means what you were
   asked for. */
function sprintCandidates() {
  const q = sp.queue[sp.idx];
  /* Typed jyutping, tone digits and all — but the digit is optional. Requiring
     it would make the mode a tone test as well as a recall test, and the tones
     are already being drilled everywhere else. */
  const t = untoned(sp.typed);
  const el = $("#spCands");
  if (!el) return;
  if (!t) { sp.cands = []; el.innerHTML = `<span class="sp-cands-hint">Start typing the sound.</span>`; return; }
  const exact = c => untoned(CHAR_INDEX[c].p) === t;
  let hits = sp.known
    .filter(c => untoned(CHAR_INDEX[c].p).startsWith(t))
...
function sprintMarked(run, best, prev) {
  ...
  $("#spInner").innerHTML = `
    <div class="sp-done">
      ...
      ${best ? `<div class="sp-record"><span class="han">新紀錄</span> Your best on this sheet${prev ? ` — was ${prev.right}/${prev.n}` : " yet"}</div>` : ""}
      <div class="sp-stats">
      ...
      ${missed.length ? `<div class="sp-wrong">
        ...
        <p class="note">${missed.length === 1 ? "It's" : "They're"} in your <span class="han">錯字本</span> now.</p>
...
/* What this mode's picker is set to — last time's sheet, or a sensible first
   one: forty questions in two minutes, which is squarely a 穩 reading sheet
