...
/* Ten characters exist only in Cantonese and are not in the Make Me a Hanzi
   corpus, which is built from fonts that predate written Cantonese being
   taken seriously as a thing to typeset. They are among the most frequent
   characters in the written language, so this is a real gap rather than an
   oversight — the app already degrades correctly, showing them without a
   stroke-order animation and keeping them out of the writing drills.
...
if (gaps.length) {
  console.log(`\n${gaps.length} character(s) have no stroke data upstream:`);
  console.log('  ' + gaps.map(p => [...p][0]).join(' ') + '  — Cantonese-only, see KNOWN_GAP');
  console.log('  they render without an animation and are excluded from writing drills');
}
