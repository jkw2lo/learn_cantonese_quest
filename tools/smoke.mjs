...
console.log('\nno drill shows a character you have not met');
{
  ...
  /* Measured past the opening stage. The first characters taught cannot have a
     pairing made of taught characters, because at that point almost nothing has
     been taught — 我 is the first character in the library and has to wait for
     哋 three places later. Hanzi Quest could ask for 90% across the whole
     library because it has 763 characters and the tail is long; here the honest
     question is whether a character arrives ready once the language has enough
     of itself to build with. */
  const past = HQ.filter(ch => ch.stage > 1);
  const nowPast = past.filter(ch => ch.words.some(w => cjk(w[0]).length > 1
    && cjk(w[0]).every(c => CHAR_INDEX[c] && CHAR_INDEX[c].i <= ch.i))).length;
  ok('most arrive with one, once past the first stage', nowPast > past.length * 0.9,
     `${nowPast} of ${past.length}`);
